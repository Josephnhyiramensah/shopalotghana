...
import {
  ...
  FiTruck, FiCheckCircle, FiXCircle, FiShoppingBag
} from "react-icons/fi"
...
function OrderCard({ order }) {
  ...
  const firstImage = order.items && order.items.length > 0 && order.items[0].image
    ? order.items[0].image
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
        <div className="flex items-center gap-3">
          <FiPackage size={18} className="text-[#FF6F00]" />
          <div>
      ...
      <div className="px-5 py-4">
        <div className="flex items-center gap-3 mb-4">
          <div className="flex -space-x-2">
            ...
            {order.items.length > 3 && (
              <div className="w-12 h-12 rounded-xl bg-gray-100 border-2 border-white
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="font-extrabold text-[#FF6F00]">
              {formatGHS(order.totalPrice)}
            </span>
            <Link
              to={"/order-confirmation/" + order._id}
              className="flex items-center gap-1 text-xs font-semibold text-[#1D3557]
                         hover:text-[#FF6F00] transition"
            >
...
function OrderSkeleton() {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 animate-pulse">
      <div className="px-5 py-4 border-b border-gray-100 bg-gray-50 flex justify-between">
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
            ...
            <button
              ...
              className={"px-4 py-2 rounded-xl text-sm font-medium transition capitalize " +
                (filter === f
                  ...
                  : "bg-white border border-gray-200 text-gray-600 hover:border-[#E63946]")}
            >
              {f}
            </button>
          )
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map(function(i) { return <OrderSkeleton key={i} /> })}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-20">
          ...
          {filter === "all" && (
            <Link
              to="/shop"
              className="inline-flex items-center gap-2 bg-[#FF6F00] hover:bg-red-700
